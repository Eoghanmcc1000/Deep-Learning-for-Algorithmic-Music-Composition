...
def _early_stopping_check(val_loss, best_val_loss, counter, patience):
    """Small helper to update early-stopping state.

    Returns: (new_best_val_loss, new_counter, should_stop)
    """
...
for iter in range(max_iters):
    ...
    if iter % eval_interval == 0 or iter == max_iters - 1:
        ...
        # use helper to manage early stopping state
        best_val_loss, counter, stop = _early_stopping_check(val_loss, best_val_loss, counter, patience)
        if stop:
    ...
    # evaluate the loss and take an optimizer step
